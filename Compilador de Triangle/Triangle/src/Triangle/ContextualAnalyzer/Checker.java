...
/*
Se agregaron las validaciones semánticas para ForCommand, RepeatCommand.
ForCommand:
-La variable de control debe de ser válida.
-El tipo de variable puede ser Integer o Char.
-Las expresiones de inicio y fin deben coincidir con el tipo de la variable de control.

RepeatCommand:
-La condición para terminar debe de ser tipo boolean.
...
public final class Checker implements Visitor {
  ...
  //agregado
    public Object visitForCommand(ForCommand ast, Object o) {
        ...
        if (!ast.V.variable) {
            ...
            reporter.reportError("Integer or Char expected here", "", ast.V.position);
        }

        TypeDenoter e1Type = (TypeDenoter) ast.E1.visit(this, null);
        if (!e1Type.equals(vType)) {
            reporter.reportError("Wrong type for starting expression (expected " + 
                               vType.getClass().getSimpleName() + ")", "", ast.E1.position);
        }

        TypeDenoter e2Type = (TypeDenoter) ast.E2.visit(this, null);
        if (!e2Type.equals(vType)) {
            reporter.reportError("Wrong type for ending expression (expected " + 
                               vType.getClass().getSimpleName() + ")", "", ast.E2.position);
        }
  ...
  //agregado
    public Object visitRepeatCommand(RepeatCommand ast, Object o) {
        ...
        if (!(condType instanceof BoolTypeDenoter)) {
            reporter.reportError("Boolean expected in until condition", 
                                "", ast.E.position);
        }
