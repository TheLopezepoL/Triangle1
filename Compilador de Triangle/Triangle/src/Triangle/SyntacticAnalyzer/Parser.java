 ...
 */
/*
  Gramática extendida para el comando 'for':
...
// to represent its phrase structure.
  ...
  Command parseSingleCommand() throws SyntaxError {
    ...
    case Token.WHILE:
      ...
      break;
    /*
      Se agregó un nuevo case para soportar el comando FOR y reconocer
      las nuevas palabras reservadas for, from, to, downto.
      Se crea el nodo para el AST
      */ 
    case Token.FOR: {
        acceptIt(); 
        ...
        if (currentToken.kind == Token.BECOMES) {
            accept(Token.BECOMES); // :=
        } else if (currentToken.kind == Token.FROM) {
            accept(Token.FROM); // from
        } else {
            System.out.println("Expected ':=' or 'from'");
        }

        Expression e1AST = parseExpression();

        boolean isDownto = (currentToken.kind == Token.DOWNTO);
        if (currentToken.kind == Token.TO || currentToken.kind == Token.DOWNTO) {
            acceptIt();
        } else {
            System.out.println("Expected 'to' or 'downto'");
        }
    ...
    break;
        /*
      Se agregó un nuevo case para soportar el comando REPEAT y reconocer
      las nuevas palabras reservadas repeat y until.
      Se crea el nodo para el AST
      */  
    case Token.REPEAT:
      {
